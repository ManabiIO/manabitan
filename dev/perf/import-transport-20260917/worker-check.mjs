...
try{
    ...
    let jobs=0
    for(const dict of [null,'jmdict'])for(const fast of [false,true])for(const workers2 of [false,true]){
        for(const w of workers)w.calls=[]
        const opts={experimentalZstdFast2:fast,experimentalCompressionWorkers2:workers2,experimentalGenericSpanCompression:true}
        ...
        assert.deepEqual(workers.map(w=>w.calls.length),workers2?[3,2,0,0]:[2,1,1,1])
        jobs+=5
        for(const w of workers)w.calls=[]
        ...
        assert.deepEqual(workers.map(w=>w.calls.length),workers2?[5,4,0,0]:[3,2,2,2])
        jobs+=9
    }
    const bad=pool.beginCompressWrappedSpans(new Uint8Array(new SharedArrayBuffer(8)),Uint32Array.of(8),Uint32Array.of(1),Uint32Array.of(0,1),Uint32Array.of(1),null,{experimentalGenericSpanCompression:true})
    await assert.rejects(bad.sourceConsumed)
    await assert.rejects(bad.completion)
    pool.close()
    const receipt={status:'success',jobs,malformedWorkerRejection:true,notes:'Actual Node worker threads load the unmodified production worker entrypoint; exact compiled codec bytes, real message transfers, shared-source consumption and packed detachment'}
