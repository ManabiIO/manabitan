...
common = [
    ...
    'ext/lib/term-bank-parser.wasm', 'ext/lib/zstd.wasm',
]
...
for pair in range(1, args.pairs + 1):
    for index in range(len(datasets)):
        ...
        for arm in order:
            schedule.append({'pair': pair, 'dataset': dataset, 'arm': arm, 'order': order})
result = {
    'label': args.label,
    ...
    'pairsPlanned': args.pairs, 'schedule': schedule, 'runs': [], 'status': 'running',
}
