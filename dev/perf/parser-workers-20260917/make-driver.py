...
old="dedupUniqueCount:parser.dedupUniqueCount,groups:parser.parserParallelGroupCount,workers:parser.parserParallelWorkerCount"
new="dedupUniqueCount:parser.dedupUniqueCount"
assert src.count(old)==1
src=src.replace(old,new)
src=src.replace("console.log(`${id}: ${actual.totalImportMs.toFixed(2)} ms; ${finalization.termContentTotalWriteBytes} content bytes`)","console.log(`${id}: ${actual.totalImportMs.toFixed(2)} ms; workers=${parser.parserParallelWorkerCount}; groups=${parser.parserParallelGroupCount}; heap=${parser.parserMaxWasmHeapBytes}`)")
Path(sys.argv[2]).write_text(src)
print('Created fixed-plan parser-worker driver; worker/group counts are observations rather than false logical-equality requirements')
