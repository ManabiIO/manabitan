...
const out = path.resolve(process.env.ZSTD_WORKERS_OUTPUT ?? 'builds/zstd-workers')
const dictionary = process.env.ZSTD_WORKERS_DICTIONARY ?? 'wty-en-en'
const variants = {
    control: {},
    workers2: {experimentalCompressionWorkers2: true},
    workers3: {experimentalCompressionWorkers3: true},
}
const selected = (process.argv[2] ?? 'control,workers2,workers3').split(',')
...
const {getExperimentalCompressionWorkerLimit, snapshotTermBankExperiments} = await import(pathToFileURL(path.join(root, 'ext/js/dictionary/term-bank-experiments.js')).href)
...
const files = ['builds/manabitan-chrome-dev.zip', 'ext/js/dictionary/dictionary-importer.js', 'ext/js/dictionary/dictionary-database.js', 'ext/js/dictionary/term-content-block-store.js', 'ext/js/dictionary/zstd-term-content.js', 'ext/js/dictionary/term-bank-experiments.js', 'types/ext/dictionary-importer.d.ts', 'test/zstd-term-content-pool.test.js', 'test/chromium/extension-two-dictionary-import.e2e.js', 'test/perf/dictionaries.lock.json', 'package-lock.json']
const identities = Object.fromEntries(files.map(p => [p, hash(p)]))
for (const id of selected) {
    const flags = snapshotTermBankExperiments(variants[id])
    const expected = id === 'workers2' ? 2 : id === 'workers3' ? 3 : null
    assert.equal(getExperimentalCompressionWorkerLimit(flags), expected)
}
const plan = []
...
try {
    for (const [index, entry] of plan.entries()) {
        ...
        const expectedLimit = getExperimentalCompressionWorkerLimit(snapshotTermBankExperiments(entry.flags))
        const actualLimit = finalization.termContentStoreDiagnostics?.blockStore?.compressionWorkerLimit ?? null
        assert.equal(actualLimit, expectedLimit, `${id}: compression worker limit did not reach block store`)
        summary.observations.push({...entry, id, totalImportMs: actual.totalImportMs, workerImportMs: actual.workerImportMs, automationObservedImportMs: actual.automationObservedImportMs, validation: actual.validation, parser, finalization, compressionWorkerLimit: actualLimit, report: path.basename(reportFile), reportSha256: hash(reportFile)})
        persist()
        console.log(`${id}: ${actual.totalImportMs.toFixed(2)} ms; limit=${actualLimit ?? 4}`)
    }
