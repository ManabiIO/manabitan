...
source = 'ext/js/dictionary/' + ('wasm/term-bank-parser.c' if kind == 'bulk-memory' else 'zstd-term-content.js')
...
assert git(b, 'diff', '--name-only', 'HEAD^', 'HEAD') == source
ids = {'A': identity(a), 'B': identity(b)}
assert not ids['A']['status'] and not ids['B']['status'], ids
allowed = [source.removeprefix('ext/')]
if kind == 'bulk-memory':
    allowed.append('lib/term-bank-parser.wasm')
...
for p in ids['A']['hashes']:
    if p not in [source, package] and not (kind == 'bulk-memory' and p == 'ext/lib/term-bank-parser.wasm'):
