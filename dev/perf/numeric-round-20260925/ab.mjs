...
const out = path.join(root, 'builds/numeric-round')
...
const identities = {}
for (const arm of ['base', 'candidate']) { identities[arm] = hash(await readFile(path.join(out, `${arm}.js`))) }
assert.notEqual(identities.base, identities.candidate)
const plan = [{kind: 'warmup', arms: ['candidate', 'base']}, {kind: 'control', arms: ['base', 'base']}, {kind: 'control', arms: ['base', 'base']}]
for (let i = 0; i < pairs; ++i) {
    plan.push({kind: 'pair', arms: i % 2 === 0 ? ['candidate', 'base'] : ['base', 'candidate']})
    if ((i + 1) % 4 === 0) { plan.push({kind: 'control', arms: ['base', 'base']}) }
}
plan.push({kind: 'control', arms: ['base', 'base']}, {kind: 'control', arms: ['base', 'base']})
const summary = {dictionary, fixture, flags, pairs, node: process.version, identities, source: await getSourceProvenance(root), plan, observations: []}
...
for (const [block, item] of plan.entries()) {
    for (const [position, arm] of item.arms.entries()) {
        ...
        await copyFile(path.join(out, `${arm}.js`), path.join(root, 'ext/js/dictionary/term-bank-wasm-parser.js'))
        assert.equal(hash(await readFile(path.join(root, 'ext/js/dictionary/term-bank-wasm-parser.js'))), identities[arm])
...
summary.result = {baselineMedian: median(armTimes.base), candidateMedian: median(armTimes.candidate), pairedPercent: median(deltas), fasterPairs: deltas.filter((x) => x < 0).length, deltas, controlDeltas, controlMedian: median(controlDeltas), controlAbsoluteMedian: median(controlDeltas.map(Math.abs))}
await writeFile(path.join(out, 'summary.json'), JSON.stringify(summary, null, 2) + '\n')
