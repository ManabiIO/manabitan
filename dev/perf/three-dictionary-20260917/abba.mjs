...
const out = path.resolve(process.env.THREE_OUTPUT ?? 'builds/three-screen')
mkdirSync(out, {recursive: true})
const dictionary = process.env.THREE_DICTIONARY ?? 'wty-en-en'
const variants = {
    control: {},
    inflate: {experimentalLibdeflate: true},
    copies: {experimentalAlignedImportCopies: true},
    both: {experimentalLibdeflate: true, experimentalAlignedImportCopies: true},
}
const selected = (process.argv[2] ?? 'control,inflate,copies,both').split(',')
...
const files = ['builds/manabitan-chrome-dev.zip', 'ext/lib/term-bank-parser.wasm', 'ext/lib/zstd-wasm.js', 'ext/js/dictionary/wasm/term-bank-parser.c', 'ext/js/dictionary/wasm/term-bank-inflate.c', 'ext/js/dictionary/term-bank-wasm-parser.js', 'ext/js/dictionary/term-bank-experiments.js', 'ext/js/core/import-byte-copy.js', 'dev/lib/zstd-wasm.js', 'ext/js/dictionary/dictionary-importer.js', 'ext/js/dictionary/dictionary-database.js', 'ext/js/dictionary/term-content-block-store.js', 'ext/js/dictionary/zstd-term-content.js', 'ext/js/dictionary/zstd-term-content-compression-worker.js', 'test/chromium/extension-two-dictionary-import.e2e.js', 'dev/perf/benchmark-support.js', 'test/perf/dictionaries.lock.json', 'package-lock.json']
...
const summary = {schemaVersion: 1, status: 'running', base: '8191f6c6997d7afc5d54e647e93af6def48dfa98', startedAt: new Date().toISOString(), dictionary, fixture, blocks, identities, driverHash: hash(process.argv[1]), node: process.version, cpus: os.cpus(), totalMemory: os.totalmem(), plan, observations: [], results: {}, rules: 'Same build, experimental flags off/on. Fresh browser and OPFS. Fixed ABBA blocks and excluded full warmups. Interleaved A/A. No retries, outlier removal, profiling or sampling. Browser file-input change through post-UI completion. No cross-host or cross-dictionary pooling.'}
...
try {
    for (const [index, entry] of plan.entries()) {
        ...
        assert.deepEqual(signature, work, 'Import work/content write totals changed')
