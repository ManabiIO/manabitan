"""Frozen whole-import A/B; all observations retained, no import retries."""
import argparse
import hashlib
import json
import math
...
def main():
    ...
    expected = {'A': 'da8f7436d34e510441d30692c8d698806b70e13f', 'B': '03478be25506176610b2133be658ee697d9bb3f1'}
    identity = {}
    for arm, root in roots.items():
        assert git(root, 'rev-parse', 'HEAD') == expected[arm]
        assert not git(root, 'status', '--porcelain')
        identity[arm] = {'commit': expected[arm], 'tree': git(root, 'rev-parse', 'HEAD^{tree}'), 'package_sha256': digest(root / 'builds/manabitan-chrome-dev.zip')}
