...
const out = path.resolve(process.env.RAWZIP_OUTPUT ?? 'builds/rawzip-slice')
const dictionary = process.env.RAWZIP_DICTIONARY ?? 'wty-en-en'
const variants = {control: {}, rawSlice: {experimentalRawZipSliceCopy: true}}
const selected = (process.argv[2] ?? 'control,rawSlice').split(',')
...
const files = ['builds/manabitan-chrome-dev.zip', 'ext/js/dictionary/dictionary-importer.js', 'ext/js/dictionary/term-bank-source-pipeline.js', 'ext/js/dictionary/term-bank-experiments.js', 'types/ext/dictionary-importer.d.ts', 'test/raw-zip-slice-copy.test.js', 'test/chromium/extension-two-dictionary-import.e2e.js', 'test/perf/dictionaries.lock.json', 'package-lock.json']
...
try {
    for (const [index, entry] of plan.entries()) {
        ...
        const parser = actual.importDebug.importerPhaseTimings.find(p => p.phase.startsWith('term-file-fast-path:'))?.details
        const finalization = actual.importDebug.importerPhaseTimings.find(p => p.phase === 'bulk-finalization')?.details
        assert.ok(parser && finalization && finalization.ok === true)
        const signature = {rows: parser.rows, banks: parser.batchedFileCount, compressed: parser.parserSourceCompressedBytes, uncompressed: parser.parserSourceUncompressedBytes, content: parser.parserEncodedContentBytes, unique: parser.dedupUniqueCount}
        if (work === null) work = signature
        assert.deepEqual(signature, work)
