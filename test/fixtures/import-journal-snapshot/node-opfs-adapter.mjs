...
export async function createNodeOpfs({terminalWriteErrors = false, invalidateSnapshots = false} = {}) {
    ...
    const operations = []
    const hooks = {beforeGetFileHandle: null, beforeRemove: null, beforeGetDirectory: null, beforeAbort: null, beforeClose: null, beforeSeek: null, beforeWrite: null, beforeGetFile: null, beforeTruncate: null, beforeCreateWritable: null, afterCreateWritable: null}
    let serial = 0
    const versions = new Map()
    const changed = (location) => versions.set(location, (versions.get(location) ?? 0) + 1)
    const snapshot = (bytes, name, location) => {
        const file = new File([bytes], name)
        if (!invalidateSnapshots) { return file }
        ...
        Object.defineProperty(file, 'arrayBuffer', {value: async () => { check(); return read() }})
        Object.defineProperty(file, 'slice', {value: (...args) => {
            const blob = slice(...args)
            const readBlob = blob.arrayBuffer.bind(blob)
            Object.defineProperty(blob, 'arrayBuffer', {value: async () => { check(); return readBlob() }})
            return blob
        }})
        return file
    }
    ...
    class Directory {
        ...
        async removeEntry(name, options = {}) {
            ...
            try { await rm(path.join(this.location, name), {recursive: options.recursive ?? false}); changed(path.join(this.location, name)) }
    ...
    class Handle {
        ...
        async getFile() {
            ...
            try { return snapshot(await readFile(this.location), this.name, this.location) }
            catch (error) { if (error.code === 'ENOENT') { throw missing() }; throw error }
        }
        async createWritable({keepExistingData = false} = {}) {
            ...
            const stream = {
                async seek(offset) { check(); await hooks.beforeSeek?.(name, offset); cursor = offset; operations.push({type: 'seek', name, offset}) },
                async truncate(size) {
                ...
                async abort() { await hooks.beforeAbort?.(name); closed = true; operations.push({type: 'abort', name}) },
            }
