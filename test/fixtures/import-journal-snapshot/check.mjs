...
const makeRecord = () => ({version: 1, sessionId: 'session-α', createdAt: 1789921902000,
    contentCheckpoint: {segments: [{fileName: 'segment-0.bin', fileLength: 20}]},
    recordCheckpoint: {shards: [{fileName: 'records-A.bin', fileLength: 40}, {fileName: 'index-A.bin', fileLength: 60}]}})
const cases = []
const unhandled = []
...
async function run(name, fn) {
    ...
    try { await fn(journal, fs); cases.push({name, status: 'passed'}) }
    catch (e) { cases.push({name, status: 'failed', message: e.message, stack: e.stack}) }
    finally { await fs.dispose() }
}
const mutations = {
    'shortened content prefix': (r) => { r.contentCheckpoint.segments[0].fileLength = 0 },
    'removed content inventory': (r) => { r.contentCheckpoint.segments.length = 0 },
    'removed record inventory': (r) => { r.recordCheckpoint.shards.splice(0) },
    'replaced record inventory': (r) => { r.recordCheckpoint = {shards: []} },
    'changed record file identity': (r) => { r.recordCheckpoint.shards[0].fileName = 'different.bin' },
    'changed session identity': (r) => { r.sessionId = 'different-session' },
    'invalid length after validation': (r) => { r.recordCheckpoint.shards[0].fileLength = -1 },
    'duplicate name after validation': (r) => { r.recordCheckpoint.shards.push({...r.recordCheckpoint.shards[0]}) },
}
for (const boundary of ['root', 'handle', 'writable']) {
    for (const [description, mutate] of Object.entries(mutations)) {
        await run(`${boundary}: snapshot survives ${description}`, async (journal, fs) => {
            ...
            if (boundary === 'root') { journal._getRoot = async () => { mutate(record); return fs.root } }
            else if (boundary === 'handle') { fs.hooks.beforeGetFileHandle = () => { mutate(record) } }
            else { fs.hooks.beforeCreateWritable = () => { mutate(record) } }
            await journal.write(record)
            assert.deepEqual(JSON.parse(await readFile(path.join(fs.rootPath, filename), 'utf8')), expected,
                'Persisted recovery instructions changed after validation')
        })
    }
}
for (const converted of [null, {}, {version: 2}, {...makeRecord(), sessionId: ''}, {...makeRecord(), recordCheckpoint: {shards: [null]}}]) {
    await run(`reject invalid serialized value ${JSON.stringify(converted)}`, async (journal, fs) => {
        ...
        let rootCalls = 0
        journal._getRoot = async () => { rootCalls++; return fs.root }
        const record = makeRecord()
...
for (const failure of ['cycle', 'throw']) {
    await run(`serialization ${failure} rejects before opening a writable`, async (journal, fs) => {
        const record = makeRecord()
        if (failure === 'cycle') { record.extra = record }
        else { record.toJSON = () => { throw new Error('serialization failed') } }
        await assert.rejects(journal.write(record))
...
for (const location of ['beforeWrite', 'beforeClose']) {
    await run(`${location} failure preserves prior committed journal`, async (journal, fs) => {
        ...
        fs.hooks[location] = () => { throw injected }
        const next = makeRecord(); next.sessionId = 'next'
...
await run('write and abort failures retain both causes', async (journal, fs) => {
    const a = new Error('write'); const b = new Error('abort')
    fs.hooks.beforeWrite = () => { throw a }
    fs.hooks.beforeAbort = () => { throw b }
...
await run('unreadable journal is not treated as missing', async (journal, fs) => {
    ...
    fs.hooks.beforeGetFileHandle = () => { throw e }
...
for (const mutate of [(r) => { r.version = 2 }, (r) => { r.sessionId = '' }, (r) => { r.createdAt = NaN },
    (r) => { r.contentCheckpoint.segments = Array(1) },
    (r) => { r.recordCheckpoint.shards[0].fileLength = Number.MAX_SAFE_INTEGER + 1 },
    (r) => { r.contentCheckpoint.segments[0].fileName = '../invalid' }]) {
...
const result = {runtime: process.version, model, boundary: 'Production journal with commit-on-close temporary-file adapter, not native OPFS',
    passed: cases.filter((x) => x.status === 'passed').length, failed: cases.filter((x) => x.status === 'failed').length, cases, unhandled}
