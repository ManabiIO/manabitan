...
async function run(name, fn) {
    try {
        ...
        cases.push({name, status: 'failed', message: error.message, stack: error.stack})
    }
}
...
function createHarness(fault = {}, title = 'A') {
    ...
    let closed = false
    const readRows = () => sql
        .prepare('SELECT id, title, published FROM dictionaries ORDER BY id')
        .all()
        .map((x) => ({...x}))
    const transport = {
        exec(input) {
            ...
            trace.push(text)
            if ((text === 'COMMIT' && fault.commit) || (text.startsWith('INSERT OR REPLACE') && fault.marker)) {
                throw injected
            }
            if (typeof input === 'string') {
                sql.exec(input)
            } else {
                sql.prepare(text).run(...input.bind)
            }
        },
    }
    Object.assign(owner, {
        ...
        _syncTermsVirtualTableFromRecordStore: async () => {
            if (fault.vtab) {
                throw injected
            }
        },
        _termContentBlockStore: {
            getDiagnostics: () => {
                if (fault.metrics) {
                    throw injected
                }
                return {compressionExperiments: {}}
            },
        },
        ...
        _deleteImportPublicationMarkerBestEffort: (id) => {
            sql.prepare('DELETE FROM dictionaryImportPublications WHERE sessionId = ?').run(id)
        },
        _clearBulkImportRuntimeCaches: () => {
            trace.push('cache-reset')
            if (fault.cache) {
                throw injected
            }
        },
        _applyRuntimePragmas: () => {
            trace.push('runtime-pragmas')
            if (fault.pragmas) {
                throw injected
            }
        },
        _closeBulkImportBlockSession: (collection) => {
            trace.push('block-close')
            if (fault.blockClose) {
                collection.push(injected)
            }
        },
        _restoreRuntimeAfterBulkImportFailure: (_, collection) => {
            trace.push('restore-runtime')
            if (fault.restore) {
                collection.push(new Error('injected:restore'))
            }
        },
        _rollbackBulkImportSqlite: (_, collection) => {
            if (!owner._bulkImportTransactionOpen) {
                return true
            }
            try {
                transport.exec('ROLLBACK')
            } catch (error) {
        ...
        _endBulkImportStoreSessions: async (_, content, records) => {
            trace.push(`end-extra:${content}:${records}`)
        },
        _quarantineBulkImportConnection: () => {
            trace.push('quarantine')
        },
    ...
    const session = new DictionaryImportSession({
        ...
        dictionarySummaryPrimaryKey: 1,
        errors,
        archiveReader: {
            close: async () => {
                trace.push('archive-close')
                if (fault.archive) {
                    throw injected
                }
            },
    ...
    const finish = () => session.finalizeBulkImport(() => {
        if (fault.checkpoint) {
            throw injected
        }
    }, summary)
...
try {
    ...
    for (const fault of [{}, {journal: true}, {pragmas: true, restore: true}, {cache: true, restore: true}, {journal: true, metrics: true}]) {
        await run(`publication control ${JSON.stringify(fault)}`, async () => {
            ...
            try {
                ...
                assert.equal(h.session.state, 'published')
                if (fault.restore) {
                    assert(h.errors[0] instanceof AggregateError)
                    assert.equal(h.errors[0].errors.length, 2)
                }
                if (fault.journal) {
                    assert.equal(h.owner._importJournalRecoveryPending, true)
                }
                if (Object.keys(fault).length === 0 || (fault.journal && !fault.metrics)) {
                    assert.equal(h.session.failed, false)
                }
            } finally {
                h.close()
            }
    ...
    const result = {
        sourcePath,
        methodSha256: createHash('sha256').update(source).digest('hex'),
    ...
    if (outputArg) {
        await mkdir(path.dirname(path.resolve(outputArg)), {recursive: true})
