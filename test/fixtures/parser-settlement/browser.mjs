...
try {
    ...
    const report = await page.evaluate(async () => {
        if (!crossOriginIsolated) { throw new Error('Shared worker-memory qualification requires isolation') }
        ...
        class GatedWorker extends EventTarget {
            constructor(url, options) {
                ...
                this.worker.addEventListener('error', (event) => this.dispatchEvent(new ErrorEvent('error', {message: event.message})))
            }
        ...
        for (const mode of ['eager', 'deferred', 'lazy', 'stored', 'deflate']) {
            for (const shape of ['all-empty', 'leading-empty', 'middle-empty', 'trailing-empty', 'malformed']) {
                ...
                const banks = shape === 'all-empty' ? ['[]', '[]', '[]', '[]'] :
                    shape === 'leading-empty' ? ['[]', '[]', row(0), row(1)] :
                        shape === 'middle-empty' ? [row(0), '[]', '[]', row(1)] :
                            shape === 'trailing-empty' ? [row(0), row(1), '[]', '[]'] : ['[]', '[,]', '[]', '[]']
                try {
                    const sources = await makeSources(banks, mode)
                    const sizes = sources.map((s) => s.uncompressedSize ?? s.byteLength)
                    const scores = []
                    const sink = (chunk) => { assert.ok(chunk.rowCount > 0); scores.push(...chunk.scoreList) }
                    let operation
                    if (mode === 'eager') { operation = parser.parseTermBankWithWasmColumnChunksParallel(sources, 3, sink, options) }
                    else if (mode === 'deferred') { operation = parser.parseTermBankWithWasmColumnChunksParallelDeferred(sources.map((s) => Promise.resolve(s)), sizes, 3, sink, options) }
                    else if (mode === 'lazy') { operation = parser.parseTermBankWithWasmColumnChunksParallelLazy(sources.map((s) => async () => s), sizes, 3, sink, options) }
                    else { operation = parser.parseTermBankWithWasmColumnChunksParallelCompressedLazy(sources.map((s) => async () => s), sizes, 3, sink, options) }
                    let failure
                    try { assert.equal(await operation, true) } catch (error) { failure = error }
                    if (shape === 'malformed') { assert.ok(failure instanceof Error) }
                    else {
                        if (failure) { throw failure }
                        assert.deepEqual(scores, shape === 'all-empty' ? [] : [0, 1])
                    }
                    empty.push({name: `${mode}: ${shape}`, passed: true})
                } catch (error) { empty.push({name: `${mode}: ${shape}`, passed: false, error: String(error)}) }
                finally { await parser.disposeParallelTermBankParser() }
            }
        }
        return {userAgent: navigator.userAgent, isolated: crossOriginIsolated, workerCount: workers, settlement, empty}
    })
