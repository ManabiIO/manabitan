...
export async function createNodeOpfs() {
    ...
    class Directory {
        ...
        async getDirectoryHandle(name, options = {}) {
            ...
            try {
                if (!(await stat(location)).isDirectory()) { throw new DOMException('Not a directory', 'TypeMismatchError') }
            } catch (error) {
        ...
        async getFileHandle(name, options = {}) {
            ...
            const location = path.join(this.location, name)
            try {
                if (!(await stat(location)).isFile()) { throw new DOMException('Not a file', 'TypeMismatchError') }
            } catch (error) {
                if (error.code !== 'ENOENT') { throw error }
                if (!options.create) { throw missing() }
    ...
    class Handle {
        ...
        async getFile() {
            ...
            try {
                return new File([await readFile(this.location)], this.name)
            } catch (error) {
                if (error.code === 'ENOENT') { throw missing() }
                throw error
            }
        ...
        async createWritable({keepExistingData = false} = {}) {
            ...
            const stream = {
                ...
                async write(value) {
                    ...
                    if (value?.type === 'write') {
                        if (value.position !== undefined) { cursor = value.position }
                        value = value.data
                    }
