...
describe('TermBankSourcePipeline', () => {
    ...
    test('creates a validated compressed import-wide plan without starting ordinary inflation', async () => {
        const files = Array.from({length: 4}, (_, index) => ({
            filename: `term_bank_${index + 1}.json`,
            compressionMethod: index === 0 ? 0 : 8,
            compressedSize: index === 0 ? 2 : 3,
            uncompressedSize: index === 0 ? 2 : 32,
            signature: 0x12345678 + index,
            encrypted: false,
            getData() {},
        }));
        const read = vi.fn(async () => new TextEncoder().encode('[]'));
        ...
        expect(plan?.files).toEqual(files);
        expect(plan?.estimatedByteLengths).toStrictEqual([2, 32, 32, 32]);
        expect(read).not.toHaveBeenCalled();
        ...
        await expect(plan?.loaders[1]()).resolves.toMatchObject({
            ...
            compressedSize: 3,
            uncompressedSize: 32,
            signature: 0x12345679,
            filename: 'term_bank_2.json',
        });
        await plan?.loaders[1]();
        expect(readCompressed).toHaveBeenCalledTimes(1);
        pipeline.releaseBatch([files[1]]);
        await plan?.loaders[1]();
        expect(readCompressed).toHaveBeenCalledTimes(2);
        await pipeline.dispose();
    });

    test('rejects ineligible or length-mismatched compressed ZIP sources', async () => {
        const makeFiles = () => Array.from({length: 4}, (_, index) => ({
            filename: `term_bank_${index + 1}.json`,
            compressionMethod: 8,
            compressedSize: 3,
            uncompressedSize: 32,
        ...
        const pipeline = new TermBankSourcePipeline({
            ...
            readCompressed: async () => new Uint8Array(2),
        });
        const plan = pipeline.createCompressedImportRunPlan(0);
        await expect(plan?.loaders[0]()).rejects.toThrow('Compressed term-bank ZIP size mismatch');
        await pipeline.dispose();

        for (const mutate of [
            (file) => { file.encrypted = true; },
            (file) => { file.compressionMethod = 9; },
            (file) => { file.signature = -1; },
            (file) => { file.uncompressedSize = Number.NaN; },
        ]) {
            const invalidFiles = makeFiles();
            mutate(invalidFiles[2]);
            const invalidPipeline = new TermBankSourcePipeline({
                termFiles: invalidFiles,
                enabled: true,
                read: async () => new TextEncoder().encode('[]'),
                readCompressed: async () => new Uint8Array(3),
            });
            expect(invalidPipeline.createCompressedImportRunPlan(0)).toBeNull();
            await invalidPipeline.dispose();
        }
    });
