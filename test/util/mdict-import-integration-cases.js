...
function listing(links, mdx = null) {
    globalThis.DOMParser = /** @type {typeof globalThis.DOMParser} */ (/** @type {unknown} */ (DOMParser));
    return controller._parseMdxListingDocument('https://example.com/dicts/', links.map((href) => `<a href="${href}">file</a>`).join(''), mdx);
}
/** @param {any} result */
function names(result) { return result?.mddLinks.map(/** @param {{fileName: string}} entry */ ({fileName}) => fileName); }

test('URL parser pairs numeric stems before sorting their split resources', () => {
    assert.deepEqual(names(listing(['Book.2024.10.mdd', 'Book.2024.mdx', 'Book.2024.mdd', 'Book.2024.2.mdd'])),
...
test('download stage preserves validated MDD volume order instead of sorting by the last suffix', async () => {
    ...
    const paths = ['Book.2024.mdd', 'Book.2024.1.mdd', 'Book.2024.9007199254740992.mdd', 'Book.2024.9007199254740993.mdd'];
    const files = await c._downloadListingFiles(paths.map((fileName) => ({fileName, url: `https://example.com/${fileName}`})), 100, () => {}, new AbortController().signal);
    assert.deepEqual(files.map(/** @param {File} file */ ({name}) => name), paths);
});
test('duplicate links to the same URL do not make a single MDX ambiguous', () => {
    assert.deepEqual(names(listing(['Book.mdx', 'Book.mdx#download', 'Book.mdd', 'Book.mdd'])), ['Book.mdd']);
});
test('same-name MDDs in other directories or origins cannot join the selected set', () => {
    assert.deepEqual(names(listing(['Book.mdx', 'Book.mdd', '../other/Book.1.mdd', 'https://other.example/dicts/Book.2.mdd', 'file:///Book.3.mdd'])), ['Book.mdd']);
});
test('a foreign MDX link does not make the local dictionary ambiguous', () => {
    assert.deepEqual(names(listing(['Book.mdx', 'https://other.example/Other.mdx', 'Book.mdd'])), ['Book.mdd']);
});
test('multiple local MDX files require an explicit choice and keep exact numeric ownership', () => {
    ...
    assert.deepEqual(names(listing(['Book.mdx', 'Book.1.mdx', 'Book.1.mdd', 'Book.mdd'], 'Book.mdx')), ['Book.mdd']);
});
test('distinct URLs for the same selected MDD path are rejected, not arbitrarily ordered', () => {
    assert.throws(() => listing(['Book.mdx', 'Book.mdd?version=1', 'Book.mdd?version=2']), /Ambiguous MDD/u);
});
test('direct URL companion discovery surfaces detected ambiguity instead of silently losing media', async () => {
    globalThis.DOMParser = /** @type {typeof globalThis.DOMParser} */ (/** @type {unknown} */ (DOMParser));
    globalThis.fetch = async () => new Response('<a href="Book.mdx">x</a><a href="Book.mdd?v=1">a</a><a href="Book.mdd?v=2">b</a>', {headers: {'content-type': 'text/html'}});
    await assert.rejects(controller._getMdxListingForUrl('https://example.com/dicts/Book.mdx', 'Book.mdx', new AbortController().signal), /Ambiguous MDD/u);
});
test('abort during companion discovery is not turned into MDX-only success', async () => {
    const abort = new AbortController();
    const reason = new Error('cancelled by test');
    globalThis.fetch = async () => { abort.abort(reason); throw reason; };
    await assert.rejects(controller._getMdxListingForUrl('https://example.com/Book.mdx', 'Book.mdx', abort.signal), reason);
});
test('unavailable optional listing still permits MDX-only fallback', async () => {
    globalThis.fetch = async () => { throw new Error('offline'); };
    assert.equal(await controller._getMdxListingForUrl('https://example.com/Book.mdx', 'Book.mdx', new AbortController().signal), null);
});

/** @param {Awaited<ReturnType<typeof createMdxImportData>>} data */
function stats(data) { return data.phaseTimings.find(({phase}) => phase === 'prepare-mdx:materialize-assets')?.details ?? {}; }
/**
...
async function convert(entries, mdd = [], options = {}) {
    ...
    return createMdxImportData('Book.mdx', options, mdx.bytes, mdd);
}
...
test('native repeated missing references are counted once per asset, not once per entry', async () => {
    const data = await convert([{key: 'a', value: '<img src="missing.png"><img src="missing.png">'}, {key: 'b', value: '<img src="missing.png">'}]);
...
test('native split-record MDD media and case-fallback references materialize without a missing count', async () => {
    ...
    const mdd = makeMdictFixture([{key: '\\Image.PNG', value: png}], {mdd: true, recordBlockSize: 7});
    const data = await convert([{key: 'cat', value: '<img src="image.png">'}], [{name: 'Book.MDD', bytes: mdd.bytes}]);
...
test('native embedded image is not misreported as missing external media', async () => {
    ...
    const data = await convert([{key: 'cat', value: `<img src="data:image/png;base64,${png}">`}]);
    assert.equal(stats(data).missingReferencedAssetCount ?? 0, 0);
});
test('native CSS dependencies contribute to missing resources', async () => {
    const mdd = makeMdictFixture([{key: 'styles/main.css', value: '.word {background: url(images/missing.png)}'}], {mdd: true, recordBlockSize: 7});
    const data = await convert([{key: 'cat', value: '<div class="word">A cat</div>'}], [{name: 'Book.mdd', bytes: mdd.bytes}]);
    assert.equal(stats(data).missingReferencedAssetCount, 1);
});
test('explicit media opt-out does not tell the user their resource set is incomplete', async () => {
    const data = await convert([{key: 'cat', value: '<img src="missing.png">'}], [], {includeAssets: false});
    assert.deepEqual(getMdictConversionWarnings(data.phaseTimings), []);
});
test('native partial record failure remains a skipped-definition warning, not a missing-resource count', async () => {
    const mdx = makeMdictFixture([{key: 'a', value: 'bad'}, {key: 'b', value: 'good'}], {compression: 'raw', recordBlockSize: 4});
    mdx.bytes[mdx.recordDataOffset] = 3; // unsupported compression on only the first record
...
test('native unresolved alias surfaces without misclassifying the target dictionary as a storage failure', async () => {
    const data = await convert([{key: 'a', value: '@@@LINK=absent'}, {key: 'b', value: 'A valid definition'}]);
...
test('real DOM note rendering treats a filename as text and keeps notices separate from errors', () => {
    const document = new DOMParser().parseFromString('<html><body><div id="notes" hidden></div><div id="dictionary-error" hidden></div></body></html>', 'text/html');
    const notes = /** @type {HTMLElement} */ (document.querySelector('#notes'));
    appendMdictConversionWarnings(notes, 'Book <b>name</b>.mdx', [{phase: 'prepare-mdx:convert-entries', details: {skippedEntryErrorCount: 1}}]);
    assert.match(notes.textContent ?? '', /Book <b>name<\/b>\.mdx/u);
    assert.equal(notes.querySelector('b'), null);
    assert.equal(/** @type {HTMLElement} */ (document.querySelector('#dictionary-error')).hidden, true);
});

/** @param {boolean} stale */
function importHarness(stale) {
    const c = Object.create(DictionaryImportController.prototype);
    globalThis.document = new DOMParser().parseFromString('<html><body><div class="mdict-import-warnings" hidden></div></body></html>', 'text/html');
    Object.assign(c, {
        _activeMdx: null, _activeImportRunGeneration: 1,
        _isImportRunCurrent: () => !stale,
        _recordImportLocalPhase() {},
        async _tryImportDictionaryOffscreen() { ++calls; return {result: {title: 'Book'}, errors: []}; },
        async _finalizeImportedDictionaryResult() { return {errors: [], importedTitle: 'Book'}; },
    });
    let calls = 0;
    Mdx.prototype.convertDictionary = async () => ({archiveContent: new ArrayBuffer(1), archiveFileName: 'Book.zip', phaseTimings: [{phase: 'prepare-mdx:convert-entries', elapsedMs: 0, details: {skippedEntryErrorCount: 1}}]});
    return {c, get calls() { return calls; }};
}
test('controller fences stale conversion before publishing notices or invoking storage', async () => {
    const h = importHarness(true);
    await assert.rejects(h.c._importDictionaryFromMdx({mdxFile: new File(['x'], 'Book.mdx'), mddFiles: []}, null, {}, false, true, 1, () => {}), /stale MDX/u);
...
test('controller publishes a conversion note without turning successful installation into an error', async () => {
    ...
    const result = await h.c._importDictionaryFromMdx({mdxFile: new File(['x'], 'Book.mdx'), mddFiles: []}, null, {}, false, true, 1, () => {});
    assert.equal(h.calls, 1);
    assert.deepEqual(result, {errors: [], importedTitle: 'Book'});
    assert.match(document.querySelector('.mdict-import-warnings')?.textContent ?? '', /Installation status is shown separately/u);
    assert.equal(h.c._activeMdx, null);
});
...
test('direct MDX download preserves its URL filename for companion discovery', async () => {
    ...
    c._getMdxListingForUrl = async (/** @type {string} */ url, /** @type {string} */ filename) => {
        assert.equal(filename, 'Book.2024.mdx');
        return {mdxLink: {url, fileName: filename}, mddLinks: [{url: url.replace('.mdx', '.mdd'), fileName: 'Book.2024.mdd'}]};
    };
    c._downloadListingFiles = async () => [new File(['mdd'], 'Book.2024.mdd')];
    const source = await c._downloadMdxImportSourceFromUrl('https://example.com/Book.2024.mdx', 100, () => {}, new AbortController().signal, 'Book.2024.mdx');
...
test('directory-listing MDX download keeps the validated filename and original bytes', async () => {
    ...
    const source = await c._downloadMdxImportSourceFromListing({mdxLink: {url: 'https://example.com/Book.mdx', fileName: 'Book.mdx'}, mddLinks: []}, 100, () => {}, new AbortController().signal);
