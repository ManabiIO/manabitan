...
export function makeMdictFixture(entries, options = {}) {
    ...
    const header = Buffer.from(`<${tag} GeneratedByEngineVersion="${version}" RequiredEngineVersion="${version}" Encrypted="${xmlAttribute(String(encrypted))}" KeyCaseSensitive="${xmlAttribute(keyCaseSensitive)}"${encodingAttribute}${formatAttribute}${styleSheetAttribute} Title="${xmlAttribute(title)}" Description="Generated regression fixture"/>\0`, 'utf16le');
