...
function source(text, method) {
    ...
    return {bytes: method === 8 ? new Uint8Array(deflateRawSync(bytes)) : bytes,
        decodedBytes: bytes.length, method, signature: crc32(bytes)}
}
...
function join(sources, spans = false, capacityOverride) {
    ...
    const length = wasm.inflate_and_join_term_banks(input, inputLength, meta,
        meta + sources.length * 4, meta + sources.length * 8,
        meta + sources.length * 12, meta + sources.length * 16,
        sources.length, output, capacity, spans ? spanPointer : 0)
    ...
    return {length, bytes: heap.slice(output, output + Math.max(0, length)),
...
describe('native join byte and ownership boundaries', () => {
    ...
    test('keeps independently bounded span output byte-identical after ordinary joins', () => {
        ...
        const saved = ordinary.bytes.slice()
