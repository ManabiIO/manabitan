...
async function walk(dir) {
    for (const e of await fs.readdir(dir, {withFileTypes: true})) {
        ...
        if (e.isDirectory()) { await walk(p); } else { const data = await fs.readFile(p); assets.push({path: path.relative(output, p).split(path.sep).join('/'), bytes: data.byteLength, sha256: createHash('sha256').update(data).digest('hex')}); }
    }
}
await walk(output);
...
await fs.writeFile(path.join(output, 'manifest.json'), JSON.stringify({package: 'manabitan-web',
    ...
    scanner: 'web/scanner.js',
    renderer: 'web/render.js',
    presets: 'web/presets.js',
    style: 'css/structured-content.css',
    assets}, null, 2));
