...
const server = http.createServer(async (req, res) => {
    ...
    if (pathname === '/default.zip') {
        try { res.writeHead(200, {'Content-Type': 'application/zip'}).end(await fs.readFile(path.join(fixtures, 'jitendex-yomitan.zip'))); } catch { res.writeHead(404).end(); } return;
    }
    ...
    try { const data = await fs.readFile(filename); res.writeHead(200, {'Content-Type': mime[path.extname(filename)] || 'application/octet-stream', 'Cache-Control': 'public, max-age=86400'}).end(data); } catch { res.writeHead(404).end(); }
});
 ...
 * @param fn
 */
async function check(name, fn) { const start = Date.now(); try { const data = await fn(); results.push({name, status: 'passed', ms: Date.now() - start, data}); console.log('PASS', name); } catch (error) { results.push({name, status: 'failed', ms: Date.now() - start, error: String(error.stack || error)}); console.error('FAIL', name, error); throw error; } finally { await save(); } }
/**
 *
 */
async function launch() {
    context = await chromium.launchPersistentContext(profile, {channel: 'chromium', headless: true}); context.setDefaultTimeout(20000); page = context.pages()[0] || await context.newPage();
...
try {
    ...
    for (const query of ['猫', '学校', '食べました']) {
        await check('real core translation ' + query, async () => {
