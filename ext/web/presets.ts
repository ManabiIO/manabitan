...
export const DEFAULT_DICTIONARY = Object.freeze({
    name: 'Jitendex', version: '2026.08.11.0', fileName: 'jitendex-2026.08.11.0.zip',
    bytes: 38698313, sha256: '8364e69e7bd0881c42011e96af921a7399d7fe06e2bf4fff4da6d18affff74fc',
    source: 'https://github.com/stephenmk/stephenmk.github.io/releases/download/2026.08.11.0/jitendex-yomitan.zip',
    license: 'CC-BY-SA-4.0', attribution: 'Jitendex by Stephen Kraus; includes JMdict, Tatoeba and JmdictFurigana data.',
...
export async function recommendedDictionaries(): Promise<RecommendedDictionary[]> {
    ...
    for (const [category, items] of Object.entries(catalog.ja)) {
        ...
        for (const item of items) {
            ...
            result.push({name: String(item.name), description: String(item.description), homepage: homepage.href,
                downloadUrl: download.href, category});
        }
    }
    return result;
}
