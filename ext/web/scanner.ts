...
export function createReaderScanner(client: ManabiTanWebClient, container: HTMLElement, callbacks: ScannerCallbacks) {
    ...
    const api: ScannerApi = {
        async termsFind(text) {
            ...
            try { return await client.lookup(text, {signal: controller.signal}); }
    ...
    const scanner = new TextScanner({api, node: container, getSearchContext: () => ({
        optionsContext: {depth: 0, url: location.href, modifiers: [], modifierKeys: [], pointerType: 'mouse'},
        detail: {documentTitle: document.title}
    }), textSourceGenerator: new TextSourceGenerator(), browser: null,
    searchTerms: true, searchKanji: false, searchOnClick: false});
    scanner.language = 'ja';
    ...
    const options: InputOptions = {
        searchTerms: true, searchKanji: false, scanOnTouchMove: false, scanOnTouchPress: false,
        scanOnTouchRelease: false, scanOnTouchTap: true, scanOnPenMove: false, scanOnPenHover: true,
        scanOnPenReleaseHover: false, scanOnPenPress: false, scanOnPenRelease: false,
        preventTouchScrolling: false, preventPenScrolling: false, minimumTouchTime: 0
    };
    ...
    ], selectText: false, normalizeCssZoom: true, delay: 50, scanLength: 32, layoutAwareScan: true, scanResolution: 'letter'});
    scanner.on('searchSuccess', (result) => { if (active) callbacks.onResult(result); });
