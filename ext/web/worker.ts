...
import {API_VERSION, STORAGE_LOCK, MAX_ARCHIVE_BYTES, WebRuntimeError, isRequest, record, text} from './protocol.js';
import type {Preferences, Request, Reply, Status} from './protocol.js';
...
let ready = false;
let closing = false;
let current: {id: number; cancelled: boolean} | undefined;
...
async function writePreferences(next: Preferences) {
    ...
    try { await writer.write(JSON.stringify(next)); await writer.close(); }
...
async function initializeStorage() {
    ...
    try {
        ...
        if (!record(value) || value.version !== 1 || !Array.isArray(value.disabled) || value.disabled.length > 4096 ||
            ...
            !(value.defaultTitle === null || typeof value.defaultTitle === 'string' && value.defaultTitle.length <= 256)) {
...
async function open() {
    ...
    await new Promise<void>((resolve, reject) => {
        navigator.locks.request(STORAGE_LOCK, {mode: 'exclusive', signal: wait.signal}, async () => {
            clearTimeout(timer);
            try { await initializeStorage(); resolve(); }
            ...
            reject(error instanceof DOMException && error.name === 'AbortError' ?
                new WebRuntimeError('storage_busy', 'The built-in dictionary is open in another Reader tab. Close its dictionary or switch that tab to extension/off, then retry.') : error);
        });
    });
    return status();
}
...
async function lookupOptions(): Promise<FindTermsOptions> {
    ...
    for (const [index, dictionary] of info.entries()) {
        if (!preferences.disabled.includes(dictionary.title) && dictionary.importSuccess !== false) {
            enabledDictionaryMap.set(dictionary.title, {index, alias: '', allowSecondarySearches: true,
                partsOfSpeechFilter: true, useDeinflections: true});
        }
    }
    return {matchType: 'exact', deinflect: true, primaryReading: '', mainDictionary: '',
        sortFrequencyDictionary: null, sortFrequencyDictionaryOrder: 'ascending',
        removeNonJapaneseCharacters: true, textReplacements: [null], enabledDictionaryMap,
        excludeDictionaryDefinitions: null, searchResolution: 'letter', language: 'ja', useAllFrequencyDictionaries: true};
}
    ...
    switch (request.operation) {
        ...
        case 'import': {
            ...
            const importer = new DictionaryImporter(new DictionaryImporterMediaLoader(),
                (progress) => reply(request.id, {progress}), () => !!current?.cancelled || closing);
            const imported = await importer.importDictionary(database, p.archive, {
                prefixWildcardsSupported: true, yomitanVersion: '0.0.0.0',
                mediaResolutionConcurrency: 2, zipMaxWorkers: 2, zipUseWebWorkers: false
            });
        ...
        case 'enable': {
            ...
            if (p.enabled) disabled.delete(title); else disabled.add(title);
...
async function drain() {
    ...
    while (pending.length) {
        ...
        } finally { ids.delete(request.id); current = undefined; }
    }
}
host.addEventListener('message', (event: MessageEvent<unknown>) => {
    ...
    if (message.operation === 'cancel') {
        ...
        if (index >= 0) {
            pending.splice(index, 1); ids.delete(message.id);
    ...
    ids.add(message.id); pending.push(message); void drain();
});
