...
import type {UrlContentManager, Content} from '../../types/ext/structured-content';
...
class ReaderMedia implements UrlContentManager {
    ...
    dispose() { this.disposed = true; this.controller.abort(); for (const url of this.created) URL.revokeObjectURL(url); this.created.clear(); }
    private url(path: string, dictionary: string) {
        ...
        if (!pending) {
            pending = this.client.media(dictionary, path, {signal: this.controller.signal}).then((data) => {
                ...
                const url = URL.createObjectURL(blob); this.created.add(url); return url;
            });
    ...
    loadMediaUrl(path: string, dictionary: string, loaded: (url: string) => void, failed: () => void) {
        void this.url(path, dictionary).then((url) => { if (!this.disposed) loaded(url); }, () => { if (!this.disposed) failed(); });
    }
...
export function renderDictionaryResults(container: HTMLElement, result: LookupResult, client: ManabiTanWebClient, lookup: (text: string) => void): () => void {
    ...
    for (const entry of result.dictionaryEntries.slice(0, 30)) {
        ...
        for (const [index, headword] of entry.headwords.entries()) {
            ...
            const ruby = document.createElement('ruby'); ruby.append(headword.term);
            if (headword.reading !== headword.term) { const rt = document.createElement('rt'); rt.textContent = headword.reading; ruby.append(rt); }
            heading.append(ruby);
        }
        article.append(heading);
        if (entry.frequencies.length) {
            const frequencies = document.createElement('p'); frequencies.className = 'frequency';
        ...
        for (const definition of entry.definitions.slice(0, 30)) {
            const label = document.createElement('p'); label.className = 'dictionary-name'; label.textContent = definition.dictionary;
            ...
            for (const value of definition.entries.slice(0, 100)) {
                ...
                else if (value.type === 'text') item.textContent = value.text;
                constrainStyles(item); list.append(item);
            }
            article.append(list);
        }
