...
export class ActionPopupController {
    ...
    _onSearchClick(e) {
        ...
        e.preventDefault();
        this._navigateFallback('/search.html?action-popup=true');
        return true;
    }
     ...
     * @param {string} url
     * @param {(event: MouseEvent|PointerEvent) => unknown} [customHandler]
     */
    _setupButtonEvents(selector, command, url, customHandler) {
        ...
        for (const node of nodes) {
            if (typeof command === 'string') {
                addPrimaryActivationEventListeners(node, (event) => {
                    void this._onButtonActivate(command, event, customHandler);
                });

                node.addEventListener('auxclick', (event) => {
                    if (event.button !== 1) { return; }
                    event.preventDefault();
                    void this._executeCommandWithFallback(command, {mode: 'newTab'});
                }, false);
            }

            if (typeof url === 'string' && node instanceof HTMLAnchorElement) {
                node.href = url;
                node.target = '_blank';
                node.rel = 'noopener';
            }
     ...
     */
    async _onButtonActivate(command, event, customHandler) {
        ...
        event.preventDefault();
        await this._executeCommandWithFallback(command, {mode});
    }
     ...
     * @returns {Promise<void>}
     */
    async _executeCommandWithFallback(command, params) {
        try {
            await this._api.commandExec(command, params);
        } catch (error) {
            const fallbackUrl = this._getFallbackUrlForCommand(command);
            if (fallbackUrl !== null) {
                this._navigateFallback(fallbackUrl);
                return;
            }
            throw error;
        }
     ...
     * @returns {?string}
     */
    _getFallbackUrlForCommand(command) {
        switch (command) {
            case 'openSearchPage':
                return '/search.html?action-popup=true';
    ...
    _setupOptions({options}) {
        ...
        for (const toggle of /** @type {NodeListOf<HTMLInputElement>} */ (document.querySelectorAll('.enable-search,.enable-search2'))) {
            ...
            this._setupToggleActivation(toggle);
     ...
     * @returns {void}
     */
    _setupToggleActivation(toggle) {
        const toggleContainer = toggle.closest('.toggle');
        if (!(toggleContainer instanceof HTMLElement)) { return; }
        if (toggleContainer.dataset.primaryActivationReady === 'true') { return; }
        toggleContainer.dataset.primaryActivationReady = 'true';

        let suppressNextClick = false;
        toggleContainer.addEventListener('pointerup', (event) => {
