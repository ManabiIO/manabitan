...
import {assert, bytesToHex} from '../../browser-util.js';
...
export class MdictMeta {
    constructor() {
        ...
        this.numWidth = 4;
        // num format
        this.numFmt = common.NUMFMT_UINT32;
        // encoding 编码
        this.encoding = '';
        // decoder 解码器
...
class MDictBase {
     ...
     * @param options
     */
    constructor(fname, source, passcode, options) {
        // mdx meta
    ...
    _decodeKeyInfo(keyInfoBuff) {
        ...
        if (this.meta.version == 2.0) {
            ...
            assert(this.keyHeader.keyInfoPackedSize == keyInfoBuff.length, `key_block_info keyInfoPackedSize ${this.keyHeader.keyInfoPackedSize} should equal to key-info buffer length ${keyInfoBuff.length}`);
